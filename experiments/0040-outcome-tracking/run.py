...
def main() -> None:
    ...
    bread_trajectory = [
        (i, r["bread"]) for i, r in enumerate(resources) if "bread" in r
    ]
    ...
    n_situations = 6
    cond_labels = KMeans(
        n_clusters=n_situations, random_state=42, n_init=10
    ...
    for si in range(n_situations):
        ...
        for bi in range(n_branches):
            ...
            stayed_crisis = sum(
                1 for idx in branch_indices if outcomes[idx]["stayed_crisis"]
            )
            bread_improved = sum(
                1 for idx in branch_indices if outcomes[idx]["bread_improved"]
            )
            bread_worsened = sum(
                1 for idx in branch_indices if outcomes[idx]["bread_worsened"]
            )
            ...
            print("      Outcomes:")
            print(
                f"        Crisis resolved: {crisis_resolved}/{n} ({resolve_rate:.0f}%)"
            )
    ...
    for si in range(n_situations):
        ...
        for bi in range(n_branches):
            ...
            print(
                f"    Branch {bi + 1} ({n} eps): resolve={resolved}/{n} improve={improved}/{n} → {decisions[dec_rep][:80]}"
            )
        print()
