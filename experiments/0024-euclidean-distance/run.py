...
def kmeans_euclidean(vectors: np.ndarray, k: int, max_iter: int = 100, seed: int = 42) -> np.ndarray:
...
def main() -> None:
    ...
    print(f"  p25={np.percentile(upper_dists, 25):.4f} p75={np.percentile(upper_dists, 75):.4f}")
    ...
    for name, vecs in [("raw", vectors), ("residual", residuals)]:
        for k in [8, 10]:
            ...
            sizes_cos = sorted([int((labels_cos == ki).sum()) for ki in range(k)], reverse=True)
            sizes_euc = sorted([int((labels_euc == ki).sum()) for ki in range(k)], reverse=True)
            print(f"\n  {name} K={k}:")
