...
CONDITION_EXTRACTORS = [
    ...
    (r"unfavorable|predatory|bad rate|poor rate|overpriced|1:1|1\.5:1|2:1", "bad_offer_present"),
]
...
BEHAVIOR_EXTRACTORS = [
    ...
    (r"I will decline|I will reject|I will ignore|I will not accept|refuse", "will_reject"),
...
def main() -> None:
    ...
    for k in [5, 8, 10, 15, 20]:
        ...
        sil = silhouette_score(projected, labels, metric="cosine") if k < len(unique_patterns) else 0
        print(f"  K={k}: sil={sil:.4f}")

    # Use K=10 for merging
    merge_k = min(10, len(unique_patterns) - 1)
    labels = KMeans(n_clusters=merge_k, random_state=42, n_init=10).fit_predict(projected)
    ...
    for ki in range(merge_k):
        ...
        cluster_patterns = [(p, pattern_counts[p]) for i, p in enumerate(unique_patterns) if labels[i] == ki]
