...
def test_status_returns_tick_alive_total(tmp_path):
    ...
    with (
        patch("scenarios.bread_economy.dashboard.EVENTS_PATH", events_file),
        patch("scenarios.bread_economy.dashboard.AGENTS_DIR", agents_dir),
        patch("scenarios.bread_economy.dashboard.TICK_PATH", tick_file),
    ):
...
def test_handler_structured_post_board(tmp_path):
    ...
    with patch("scenarios.bread_economy.dashboard.HANDLER_FILE", handler_file):
